...
button.addEventListener("click", () => {
  // переход на сцену 2
  window.location.href = "https://vg-mu.vercel.app/";
});
...
function drawText() {
    ...
    // glow effect
    context.shadowColor = "rgba(242, 67, 224, 1)";
    context.shadowBlur = 8;
    context.shadowOffsetX = 0;
    context.shadowOffsetY = 0;

    if(frameNumber < 250){
        context.fillStyle = `rgba(242, 67, 224, ${opacity})`;
    ...
    if(frameNumber > 2500 && frameNumber < 99999){
        ...
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks(["I love you so much Sona, more than", "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText("I love you so much Sona, more than all the time and space in the universe can contain", canvas.width/2, canvas.height/2);
        }
    ...
    if(frameNumber >= 2750 && frameNumber < 99999){
        context.fillStyle = `rgba(242, 67, 224, ${secondOpacity})`;
    ...
    if(frameNumber >= 3000 && frameNumber < 99999){
        context.fillStyle = `rgba(242, 67, 224, ${thirdOpacity})`;
        context.fillText("Press the button and let’s begin 💗", canvas.width/2, (canvas.height/2 + 120));
